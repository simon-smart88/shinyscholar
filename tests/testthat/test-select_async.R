if (suggests){
  test_that("Check select_async function works as expected", {
    ...
    if (!is.null(check_live)){
      ...
      expect_error(select_async("poly", date = "2023-06-20", token = token), "poly must be a matrix")
      expect_error(select_async(poly_matrix, date = "2023-16-20", token = token), "date must be a string with the format YYYY")
      expect_error(select_async(poly_matrix, date = 2023, token = token), "date must be a string with the format YYYY")
      expect_error(select_async(poly_matrix, date = "2023-06-20", token = NULL), "This function requires a NASA token")
      expect_error(select_async(poly_matrix, date = "2023-06-20", token = "not_a_token"), "This function requires a NASA token")
